...
secondLargestReturnType<int> *getSecondLargestNodeHelper(TreeNode<int> *root)
{
    ...
    for (int i = 0; i < root->children.size(); i++)
    {
        secondLargestReturnType<int> *temp = getSecondLargestNodeHelper(root->children[i]);
        ...
        {
            if ((ans->m != temp->m) && ((ans->sm == NULL) || (temp->m->data > ans->sm->data)))
            {
                ans->sm = temp->m;
            }
        }
...
{
    return getSecondLargestNodeHelper(root)->sm;
}
...
int main()
{
    ...
    // cout << "Second Largest Node " << secondLargestNode->data << endl;
    // int n;
